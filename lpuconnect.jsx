...
function MatchingScreen({ userEmail, onMatched, onCancel }) {
  ...
  const [queuePos, setQueuePos] = useState(Math.floor(Math.random() * 8) + 2);

  useEffect(() => {
    const d = setInterval(() => setDots(p => p.length >= 3 ? "." : p + "."), 500);
    // Simulate match after 2–4s
    const t = setTimeout(() => {
      const names = ["Student-419", "Student-073", "Student-881", "Student-256", "Student-534"];
      onMatched(names[Math.floor(Math.random() * names.length)]);
  ...
  useEffect(() => {
    if (queuePos > 1) {
      const t = setTimeout(() => setQueuePos(p => p - 1), 800);
    ...
    <div style={{
      ...
      <div style={{ position: "relative", zIndex: 1, textAlign: "center" }}>
        ...
        <p style={{ color: "#555", fontSize: 12, margin: "0 0 32px", letterSpacing: "0.08em" }}>
          {queuePos > 1 ? `${queuePos} students ahead in queue` : "Almost there..."}
        </p>
...
// ─── SCREEN: CHAT ─────────────────────────────────────────────────────────────
function ChatScreen({ userEmail, partner, onSkip, onEnd }) {
  const [messages, setMessages] = useState([
    { from: "system", text: `Connected with ${partner}. Say hello!`, time: new Date() }
  ]);
  ...
  // Simulate partner typing + responding
  useEffect(() => {
    const responses = [
      "hey! which branch are you from?",
      "lol same hostel vibes at 2am",
      "bro are you from cse or what",
      "this app is actually pretty cool ngl",
      "ok I was NOT expecting to find this 😭",
    ];
    let idx = 0;
    const typeAndSend = () => {
      if (idx >= responses.length) return;
      const delay = 4000 + idx * 3000;
      setTimeout(() => {
        setBotTyping(true);
        setTimeout(() => {
          setBotTyping(false);
          setMessages(m => [...m, { from: "partner", text: responses[idx], time: new Date() }]);
          idx++;
  ...
  // Simulate incoming video request
  useEffect(() => {
    const t = setTimeout(() => setIncomingVideo(true), 18000);
    return () => clearTimeout(t);
  }, []);
  ...
  function startVideo() {
    ...
    setTimeout(() => {
      ...
      setMessages(m => [...m, { from: "system", text: `${partner} accepted the video call.`, time: new Date() }]);
    }, 2000);
  }
  ...
  function acceptIncoming() {
    ...
    setVideoState("active");
    setMessages(m => [...m, { from: "system", text: `Video call started with ${partner}.`, time: new Date() }]);
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1 }}>
          <div style={{ color: "#f0ede8", fontSize: 14, fontWeight: 700 }}>{partner}</div>
      ...
      {videoState === "active" && (
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ position: "absolute", bottom: 8, left: 8, fontSize: 9, color: "#555" }}>You</div>
          </div>
        </div>
      )}
      {videoState === "active" && (
      ...
      <div style={{
        ...
        {messages.map((msg, i) => (
          <div key={i} style={{
            ...
            {msg.from !== "me" && msg.from !== "system" && <Avatar name={partner} size={28} />}
            <div style={{
              maxWidth: "72%",
              ...
              {msg.text}
              {msg.from !== "system" && (
                <div style={{ fontSize: 9, color: msg.from === "me" ? "rgba(0,0,0,0.4)" : "#333", marginTop: 4, textAlign: "right" }}>
                  {fmt(msg.time)}
                </div>
              )}
            </div>
      ...
      {incomingVideo && videoState !== "active" && (
        <div style={{
          ...
          <div style={{
            ...
            <Avatar name={partner} size={64} />
            <p style={{ color: "#f0ede8", marginTop: 16, marginBottom: 4, fontSize: 15, fontWeight: 700 }}>
              Incoming Video Call
            </p>
            <p style={{ color: "#555", fontSize: 12, marginBottom: 28 }}>{partner} wants to video chat</p>
            <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
              <button onClick={() => setIncomingVideo(false)} style={{
          ...
          <div style={{
            ...
            {reported ? (
              <div>
                ...
                <button onClick={() => { setShowReport(false); onSkip(); }} style={{
...
export default function App() {
  ...
  function handleMatched(partnerName) {
